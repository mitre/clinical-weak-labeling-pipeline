...
def load_negex_model(model = "en_core_sci_lg", language = 'en_clinical', chunks = ["no"], replace = False, pseudo_negations=list(),
    ...
    negex = Negex(nlp,language = language, chunk_prefix = chunks, pseudo_negations = pseudo_negations,
...
class KeywordProcess():
    ...
    def __init__(self, string, nlp, sent_filter=None):
        ...
        self.doc = nlp(string.lower())
        
        if self.sent_filter:
            self.sentences = []
            for sent in self.doc.sents:
                if re.search(sent_filter, sent.text):
                    self.sentences.append(nlp(sent.text))
            ...
            if token1:
                ...
                if neg == False:
                    if sentence[i].dep_ == 'ROOT':
                        ...
                        ancestors1.extend([t.i for t in sentence[i].ancestors])
                    
        for i in range(len(sentence)):   
            token2,j = self.find_token(sentence,i,wordlist2)
            if token2:
                neg = self.is_negated(sentence, token2,j,3)
                if neg == False:
                    if sentence[j].dep_ == 'ROOT':
                        out +=1
                    else:
                        ancestors2.extend([t.i for t in sentence[j].ancestors])
